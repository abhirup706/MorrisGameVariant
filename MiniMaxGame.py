...
def MiniMaxGame(input_file,out_file,depth):
...
def MiniMax(depth,is_white,initial_board):
    ...
    if(depth == 0):
        output_estimate = Utility.static_estimation_midgame_endgame(initial_board)
...
def main(input_file,out_file,depth):
    start = time.time()
    MiniMaxGame(input_file,out_file,depth)
